...
from dataforge.config import get_app_config, S3Config
...
class S3Loader:
    ...
    def __init__(
        ...
        if bucket_name:
            self.config = self.config.model_copy(update={"bucket_name": bucket_name})
...
def create_s3_loader(
    ...
    """
    return S3Loader(config=config, bucket_name=bucket_name)
